...
//Date : avril 2021
//version : 1.1
//Correction pour réception d'une commnande vide : "\n" seulement
...
bool Decodeur::lireBuffer(){
 _IncomingByte = -1;
  while ((_MyStream->available()>0) && (_IncomingByte != '\n')) {
    ...
    if(_InComIndex >= _maxComLine){  		//Pour éviter le débordement,
  ...
  if(_IncomingByte == '\n'){			
	  if(_InComIndex < 2){					//Correction pour une réception avec \n seulement
